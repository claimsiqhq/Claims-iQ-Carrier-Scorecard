...
export type CarrierCategoryId =
  (typeof CARRIER_SCORECARD_CATEGORIES)[number]["id"];

const CATEGORY_IDS = CARRIER_SCORECARD_CATEGORIES.map((c) => c.id) as [
  CarrierCategoryId,
  ...CarrierCategoryId[],
];
...
function buildCarrierScorecardRawSchema(
...
export const carrierScorecardNormalizedSchema = z
  .object({
...
export type CarrierScorecardAuditResult = z.infer<
  typeof carrierScorecardNormalizedSchema
>;
...
function getGrade(
  percent: number,
): CarrierScorecardAuditResult["overall"]["grade"] {
...
export async function runCarrierScorecardAudit(input: {
  ...
  try {
    ...
    const systemPrompt = [UNTRUSTED_SOURCE_GUARDRAIL, configuredPrompt].join(
      "\n\n",
    );
