...
- **Audit Generation**: Dual DA/FA carrier-grade scorecard system with root-issue grouping and anti-double-penalization. LLM answers 11 canonical questions: 7 for DA, 4 for FA. Each question returns `root_issue` (snake_case grouping key) so related findings share the same root cause. Scoring engine: (1) policy provision guard — FAIL→PARTIAL unless explicitly missing; (2) root-issue dedup — secondary questions sharing a root_issue are damped to PARTIAL; (3) materiality — non-material root issues have primary softened to PARTIAL; (4) readiness considers critical validation + material failures. Validation engine uses section-aware payment mismatch detection, deductible inconsistency (now "critical"), and existing regex checks. Output: `root_issue_groups[]` (grouped by root cause with affects/fix/impact), `issues[]` (per-question with root_issue), `validation_checks[]`. Services: `questionBank.ts`, `scoringEngine.ts`, `rootIssueEngine.ts` (grouping + materiality), `runQuestionAudit.ts`, `validationEngine.ts`, `prompts.ts`, `audit.ts`.
- **Batch Upload Queue**: Dashboard supports multi-file upload with sequential processing. Drop or select multiple PDFs, configure an optional email recipient, and start the pipeline — each file is processed through ingest → audit → email sequentially. Per-file progress tracking with statuses (queued/extracting/parsing/auditing/emailing/complete/error). Claims are persisted to the database as they complete, so navigating away preserves the data.
