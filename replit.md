...
- **Carrier Management UI**: Full carrier management interface at `/carriers` route. Lists all carriers (active/inactive) with question counts and category counts. Carrier editor at `/carriers/:key` with collapsible sections for: basic info (name, logo URL, active toggle), system prompt override, scorecard prompt override, DA questions table (editable: text, ID, weight, weightIfNoDenial, categoryKey, categoryName), FA questions table, and scorecard categories (ID, label, max_score). Supports add/remove rows for questions and categories. "Add Carrier" flow generates carrier_key from display name. Delete with confirmation modal. API: `GET /carriers/all` (all carriers), `DELETE /carriers/:key`. Navigation: sidebar "Carriers" link with Building icon.
